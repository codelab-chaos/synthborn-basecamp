 ...
 *
 * Two entry points:
 *   - CLI: `node tools/refs/sdk/build-sdk-llms-txt.js [--out <dir>]`
...
function buildLlmsTxt({ outDir = DEFAULT_REF_DIR, quiet = false, packages, version } = {}) {
  ...
  lines.push("");
  lines.push("Regenerate per-package files + indexes: `node tools/refs/sdk/extract-sdk-reference.js --full` (slow — see tools/refs/sdk/README.md)");
  lines.push("Regenerate indexes only:                  `node tools/refs/sdk/build-sdk-llms-txt.js` + `node tools/refs/sdk/build-sdk-method-index.js`");
  lines.push("Search:                                   `node tools/refs/sdk/sdk-search.js --method <name>`");
  lines.push("");
...
if (require.main === module) {
  try {
    ...
    if (opts.help) {
      console.log(`Usage: node tools/refs/sdk/build-sdk-llms-txt.js [--out <dir>]`);
      process.exit(0);
    }
    buildLlmsTxt(opts);
  } catch (err) {
