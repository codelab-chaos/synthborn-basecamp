 ...
 *
 * Usage:
 *   node tools/refs/assets/build-assets-toc.js                 # zip from the local install, auto-detect version
 *   node tools/refs/assets/build-assets-toc.js --dir _Assets   # build from an already-extracted folder
 *   node tools/refs/assets/build-assets-toc.js --zip /path/Assets.zip --version 0.5.4 --out toc.json
 *
...
function main() {
  ...
  if (args.help) {
    console.log('Usage: node tools/refs/assets/build-assets-toc.js [--zip <path>|--dir <path>] [--version <v>] [--out <path>] [--game <dir>]');
    return;
  }
  ...
  const out = args.out || path.join(BASECAMP_ROOT, 'docs', 'refs', 'assets', 'toc', `assets-toc-${version}.json`);
