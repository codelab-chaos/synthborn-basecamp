 ...
 *
 *   docs/refs/prefabs/README.md           - browsable category-grouped markdown for humans
 *                                       and for the LLM to read as catalog context.
 *   docs/refs/prefabs/prefabs-index.json  - flat array of per-prefab metadata, suitable for
 ...
 *   node tools/refs/prefabs/index-hytale-prefabs.js --root /custom/path/to/Prefabs
 *   node tools/refs/prefabs/index-hytale-prefabs.js --md-out docs/refs/prefabs/README.md
 *   node tools/refs/prefabs/index-hytale-prefabs.js --json-out docs/refs/prefabs/prefabs-index.json
 */
...
const DEFAULT_MD = path.join(REPO_ROOT, "docs/refs/prefabs/README.md");
const DEFAULT_JSON = path.join(REPO_ROOT, "docs/refs/prefabs/prefabs-index.json");

const TOP_BLOCKS_PER_PREFAB = 5;

function parseArgs(argv) {
  const opts = { root: DEFAULT_ROOT, mdOut: DEFAULT_MD, jsonOut: DEFAULT_JSON, pluginJsonOut: null };
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--plugin-json-out") opts.pluginJsonOut = path.resolve(argv[++i]);
    else if (a === "-h" || a === "--help") {
      console.log(`Usage: node tools/refs/prefabs/index-hytale-prefabs.js [--root PATH] [--md-out FILE] [--json-out FILE] [--plugin-json-out FILE]`);
      process.exit(0);
    } else {
...
function fmtSizeRange(sMin, sMax) {
  ...
  if (sMin[0] === sMax[0] && sMin[1] === sMax[1] && sMin[2] === sMax[2]) {
    return `${sMin[0]}x${sMin[1]}x${sMin[2]}`;
  }
  return `${sMin[0]}x${sMin[1]}x${sMin[2]} -> ${sMax[0]}x${sMax[1]}x${sMax[2]}`;
}
...
function buildMarkdown(prefabs) {
  ...
  lines.push("");
  lines.push(`Generated by \`tools/refs/prefabs/index-hytale-prefabs.js\` on ${new Date().toISOString()}.`);
  ...
  lines.push("");
  lines.push("- A flat per-prefab index is written alongside this file at `docs/refs/prefabs/prefabs-index.json` for runtime queries.");
...
function main() {
  ...
  if (opts.pluginJsonOut) {
    fs.mkdirSync(path.dirname(opts.pluginJsonOut), { recursive: true });
    fs.copyFileSync(opts.jsonOut, opts.pluginJsonOut);
    console.log(`Wrote plugin copy: ${opts.pluginJsonOut}`);
  }
}
