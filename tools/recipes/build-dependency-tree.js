#!/usr/bin/env node
/*
 * Build transitive crafting dependency trees from docs/recipes/recipes.json.
 *
 ...
 *   node tools/recipes/build-dependency-tree.js --prefix Tool_ --prefix Weapon_Sword_
 *   node tools/recipes/build-dependency-tree.js --all
 *   node tools/recipes/build-dependency-tree.js --include-salvage --max-depth 8
 */
...
const DEFAULT_RECIPES = path.join(REPO_ROOT, "docs", "recipes", "recipes.json");
const DEFAULT_OUT_DIR = path.join(REPO_ROOT, "docs", "recipes");
...
function usage() {
  console.log(`Usage: node tools/recipes/build-dependency-tree.js [options]
...
function readRecipeIndex(file) {
  if (!fs.existsSync(file)) {
    throw new Error(`Recipe index not found: ${file}. Run node tools/recipes/extract-recipes.js first.`);
  }
...
function renderMarkdown(payload) {
  ...
  lines.push("node tools/recipes/build-dependency-tree.js");
  lines.push("```");
  lines.push("");
