 ...
 *
 * Reads docs/labels/labels.json (node tools/labels/extract-labels.js).
 *
 * Subcommands:
 ...
 *   node tools/labels/lookup.js Wood_Beech_Trunk
 *
...
const LABELS_FILE = path.join(REPO_ROOT, "docs", "labels", "labels.json");
...
function loadLabels() {
  if (!fs.existsSync(LABELS_FILE)) {
    throw new Error(
      `${path.relative(REPO_ROOT, LABELS_FILE)} not found. Run: node tools/labels/extract-labels.js`,
    );
  }
...
function usage() {
  console.log(`Usage:
  node tools/labels/lookup.js id <token>
  node tools/labels/lookup.js name <token>
  node tools/labels/lookup.js find <token>
  node tools/labels/lookup.js <token>          # same as find

Regenerate index: node tools/labels/extract-labels.js`);
}
