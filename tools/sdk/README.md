...
[`docs/sdk-reference/`](../../docs/sdk-reference/) — one markdown file per package plus search indexes.
...
Or grep the generated indexes in `docs/sdk-reference/`:

- `llms.txt` — classes by package
- `methods.txt` — tab-separated method → class → package → file

Topic entry points: [`docs/sdk-reference/README.md`](../../docs/sdk-reference/README.md).
...
node tools/sdk/diff-sdk-reference.js
```

The extractor reads the jar version pinned in sibling mod repos (via
[`tools/library/workspace.js`](../library/workspace.js)) and picks the matching file from
...
| Stamp | `docs/sdk-reference/.sdk-source.json` updated with jar fingerprint |
...
| `--full` | ~915 auto-discovered | **Default for repo commits** — matches current `docs/sdk-reference/` |

### Rebuild indexes only (fast)

```bash
node tools/sdk/build-sdk-llms-txt.js
node tools/sdk/build-sdk-method-index.js
```
...
HYTALE_SERVER_JAR=/path/to/Server-0.5.4.jar node tools/sdk/extract-sdk-reference.js --full

# diff against another git ref or directory
node tools/sdk/diff-sdk-reference.js --against main
node tools/sdk/diff-sdk-reference.js --against /path/to/old-sdk-reference
```
...
| [`list-hytale-server-api.ps1`](list-hytale-server-api.ps1) | Windows helper — list class names in a package |
