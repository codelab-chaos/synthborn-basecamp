import type { UnpackedVoxels, VoxelRef } from "./types";
...
function isVoxelRef(value: string | VoxelRef): value is VoxelRef {
  return typeof value === "object" && value !== null && "pack" in value;
}
...
async function loadPackSlice(ref: VoxelRef): Promise<ArrayBuffer> {
  let packPromise = packCache.get(ref.pack);
  if (!packPromise) {
    packPromise = fetch(ref.pack).then((response) => {
      if (!response.ok) throw new Error(`${ref.pack} ${response.status}`);
      return response.arrayBuffer();
    });
    packCache.set(ref.pack, packPromise);
  }

  const pack = await packPromise;
  return pack.slice(ref.offset, ref.offset + ref.length);
}

function decodeVoxelBuffer(buffer: ArrayBuffer, source: string): UnpackedVoxels {
  ...
  throw new Error(`Unsupported voxel payload: ${source}`);
}

export async function loadVoxelPayload(voxelRef: string | VoxelRef): Promise<UnpackedVoxels> {
  const buffer = isVoxelRef(voxelRef) ? await loadPackSlice(voxelRef) : await fetch(voxelRef).then(async (response) => {
    if (!response.ok) throw new Error(`${voxelRef} ${response.status}`);
    return response.arrayBuffer();
  });

  const source = isVoxelRef(voxelRef) ? `${voxelRef.pack}@${voxelRef.offset}` : voxelRef;
  return decodeVoxelBuffer(buffer, source);
}
