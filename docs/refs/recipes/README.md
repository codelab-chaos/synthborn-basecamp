...
```powershell
node tools/refs/recipes/gamedata.js <command> <id> [--json]
...
|------|--------|------------|
| `recipes.json` / `.txt` | `_Assets/.../Recipes` + embedded item `Recipe` | `node tools/refs/recipes/extract-recipes.js` |
| `loot.json` / `.txt` | `_Assets/Server/Drops` + block `BlockType.Gathering` | `node tools/refs/recipes/extract-loot.js` |
| `crafting-tech-tree.*`, `equipment-tech-tree.*` | `recipes.json` | `node tools/refs/recipes/build-dependency-tree.js [--all]` |
