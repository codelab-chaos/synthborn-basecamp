...
Модороби в основному взаємодіятимуть із серверними компонентами Hytale. This means that mods will often focus on altering game logic, rules, and behaviors that are managed by the server. Client-side modifications are not allowed (as of the information given right now).
